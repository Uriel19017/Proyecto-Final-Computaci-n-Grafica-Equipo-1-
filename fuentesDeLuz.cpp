...
glm::vec3 pointLightPositions[] = {
	...
	glm::vec3(-5.0f,3.0f, -2.8f),
	glm::vec3(5.0f,3.0f,  -6.0f),
	glm::vec3(2.5f,2.2f, 3.0f)
};
...
int main()
{
	...
	Model lamp((char*)"Models/sconce_02.obj");
	Model lamp2((char*)"Models/streetlamp.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "pointLights[1].ambient"), 0.02f, 0.02f, 0.02f);
		...
		glUniform3f(glGetUniformLocation(lightingShader.Program, "spotLight.ambient"), 0.2f, 0.2f, 0.9f);
		...
		model = glm::scale(model, glm::vec3(3.0f, 0.0f, 3.0f)); // Escalar para cubrir más área
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		Piso.Draw(lightingShader);


		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(0.0f, 0.35f, 0.0f)); // Ajustar la altura según sea necesario
		...
		{
			model = glm::mat4(1);
			//glm::vec3 extraTranslation(6.0f, 3.0f, -6.0f); // Coordenadas adicionales
			//model = glm::translate(model, pointLightPositions[i] + extraTranslation);
			model = glm::translate(model, pointLightPositions[i]);
			model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f)); // Make it a smaller cube
			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
			glBindVertexArray(VAO);
			lamp.Draw(lightingShader);
